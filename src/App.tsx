...
// premier essai qui avait révélé deux bugs de synchro (déclencheurs incomplets +
// absence d'updatedAt sur les comptes) : les trois interfaces ci-dessous portent donc
// `updatedAt` dès la création, indispensable pour que mergeById puisse arbitrer un
// conflit par la date plutôt que de faire gagner arbitrairement "le distant".
interface Fund {
  id: string;
  name: string;
...
function BourseTab({ funds, setFunds, fundOperations, setFundOperations, fundDailyValues, setFundDailyValues, accounts, deletedFundIds, setDeletedFundIds, deletedFundOperationIds, setDeletedFundOperationIds, isMobile }: {
  ...
  const saveVl = () => {
    ...
    if (existing) {
      setFundDailyValues(fundDailyValues.map((v) => (v.id === existing.id ? { ...v, vl: vlValue, updatedAt: new Date().toISOString() } : v)));
    } else {
